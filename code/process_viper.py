...
# preprocessing for viper, as fine tuning data and testing data
# viper has 632 identities in total, 316 for training, 316 for testing
...
def generate_cumarket(cuhk03_file, market_train_file, market_test_file):
    ...
    # remove images of batch 4 and 5
...
def generate_viper(viper_a_file, viper_b_file, train_data_mean, current_seed):
    print('processing viper...')
    cam_a = os.listdir(viper_a_file)
    cam_b = os.listdir(viper_b_file)
    cam_a = sorted(cam_a)
    cam_b = sorted(cam_b)
    ...
    for image_path in cam_a:
        image = plt.imread(viper_a_file+image_path)
        images_a.append(image)
    for image_path in cam_b:
        image = plt.imread(viper_b_file+image_path)
        images_b.append(image)
    
    images_a = np.array(images_a)
    images_b = np.array(images_b)
    # the original scale is 1-255, to normalize it to 0-1
    ...
    # randomly select 316 as unlabeled data, other 316 as test data
    np.random.seed(current_seed)
    train_index = np.random.choice(range(images_a.shape[0]), size=316, replace=False)
    test_index = [x for x in range(images_a.shape[0]) if x not in train_index]
    images_a_train = images_a[train_index,:]
    images_b_train = images_b[train_index,:]
    images_a_test = images_a[test_index,:]
    images_b_test = images_b[test_index,:]

    viper = (images_a_train, images_b_train, images_a_test, images_b_test)
    return viper
